...
public class DeptDao {
	...
	public List<DeptVo> findAll(){
		...
		try(MongoClient client = new MongoClient(seeds);){
			...
			while(cur.hasNext()) {
				...
				list.add(new DeptVo(
						doc.getObjectId("_id"), doc.getInteger("deptno"), doc.getString("dname"), doc.getString("loc")
						));
			}			
		}
	...
	public void addOne(int deptno, String dname, String loc) {
		try(
			...
			MongoCollection<Document> coll = client.getDatabase(DATABASENAME).getCollection(COLNAME);
	...
	public DeptVo findOne(String id) {
		...
		try(
			...
			coll = client.getDatabase(DATABASENAME).getCollection(COLNAME);
			Bson filter = Filters.eq("_id", _id);
			
			FindIterable<Document> ite = coll.find(filter);
			Document doc = ite.first();
			bean = new DeptVo(doc.getObjectId("_id"), doc.getInteger("deptno"), doc.getString("dname"), doc.getString("loc"));
	...
	public long updateOne(String id, int deptno, String dname, String loc) {
		try(
			...
			coll = client.getDatabase(DATABASENAME).getCollection(COLNAME);
			log.info(id);
			Bson update = Updates.combine(Updates.set("dname", dname), Updates.set("loc", loc));
